...
this.lock_form(this.gui_objects.importform,!0)}else this.goto_url("import",this.env.source?"_target="+urlencode(this.env.source)+"&":"");break;case "export":0<this.contact_list.rowcount&&this.goto_url("export",{_source:this.env.source,_gid:this.env.group,_search:this.env.search_request},!1,!0);break;case "export-selected":0<this.contact_list.rowcount&&this.goto_url("export",{_source:this.env.source,_gid:this.env.group,_cid:this.contact_list.get_selection().join(",")},!1,!0);break;case "upload-photo":this.upload_contact_photo(b||
this.gui_objects.uploadform);break;case "delete-photo":this.replace_contact_photo("-del-");break;case "preferences":case "identities":case "responses":case "folders":this.goto_url("settings/"+a);break;case "undo":this.http_request("undo","",this.display_message("","loading"));break;default:h=a.replace(/-/g,"_"),this[h]&&"function"===typeof this[h]&&(g=this[h](b,d,e))}l||!1!==this.triggerEvent("after"+a,b)||(g=!1);this.triggerEvent("actionafter",{props:b,action:a,aborted:l,ret:g});return!1===g?!1:
d?!1:!0};this.enable_command=function(){var a,b,d=Array.prototype.slice.call(arguments),e=d.pop(),g;for(b=0;b<d.length;b++)if(g=d[b],"string"===typeof g)this.commands[g]=e,this.set_button(g,e?"act":"pas"),this.triggerEvent("enable-command",{command:g,status:e});else for(a in g)d.push(g[a])};this.command_enabled=function(a){return this.commands[a]};this.set_busy=function(a,b,d){a&&b?(d=this.get_label(b),d==b&&(d="Loading..."),d=this.display_message(d,"loading")):!a&&d&&this.hide_message(d);this.busy=
...
if(this.env.sort_col!=b||this.env.sort_order!=d)g=1,this.set_list_sorting(b,d);this.env.threading!=e&&(g=1,h._threads=e);if(a&&a.length){var f,l,m=[],n=this.env.listcols;for(e=0;e<n.length;e++)l=n[e],f=$.inArray(l,a),-1!=f&&(m.push(l),delete a[f]);for(e=0;e<a.length;e++)a[e]&&m.push(a[e]);m.join()!=n.join()&&(g=1,h._cols=m.join(","))}g&&this.list_mailbox("","",b+"_"+d,h)};this.show_message=function(a,b,d){if(a){var e,g=window,h=this.params_from_uid(a,{_caps:this.browser_capabilities()});d&&(e=this.get_frame_window(this.env.contentframe))&&
(g=e,h._framed=1);b&&(h._safe=1);this.env.search_request&&(h._search=this.env.search_request);this.env.extwin&&(h._extwin=1);h=this.url(d?"preview":"show",h);d&&0<=String(g.location.href).indexOf(h)?this.show_contentframe(!0):(d||!this.env.message_extwin||this.env.extwin?this.location_href(h,g,!0):this.open_window(h,!0),d&&this.message_list&&this.message_list.rows[a]&&this.message_list.rows[a].unread&&0<this.env.preview_pane_mark_read&&(this.preview_read_timer=setTimeout(function(){f.set_unread_message(a,
f.env.mailbox);f.http_post("mark",{_uid:a,_flag:"read",_mbox:f.env.mailbox,_quiet:1})},1E3*this.env.preview_pane_mark_read)))}};this.set_unread_message=function(a,b){var d=this;d.message_list||(d=d.opener());!d&&window.parent&&(d=parent.rcmail);d&&d.message_list&&(!1===d.set_message(a,"unread",!1)&&d.set_message(a+"-"+b,"unread",!1),0<d.env.unread_counts[b]&&(d.env.unread_counts[b]-=1,d.set_unread_count(b,d.env.unread_counts[b],"INBOX"==b&&!d.is_multifolder_listing())))};this.show_contentframe=function(a){var b,
...
1,null,{_clear:1},!0);this.message_list&&this.message_list.clear_selection()};this.list_mailbox=function(a,b,d,e,g){var h=window;"object"!=typeof e&&(e={});a||(a=this.env.mailbox?this.env.mailbox:"INBOX");d&&(e._sort=d);this.env.mailbox!=a?(b=1,this.env.current_page=b,this.env.search_scope="base",this.select_all_mode=!1,this.reset_search_filter()):this.env.search_request&&(e._search=this.env.search_request);if(!g){this.clear_message_list();if(a!=this.env.mailbox||a==this.env.mailbox&&!b&&!d)e._refresh=
...
this.selection_post_data();a._uid&&(this.show_contentframe(!1),this._with_selected_messages("delete",a))};this._with_selected_messages=function(a,b,d){var e=0,g="delete"==a||!this.is_multifolder_listing();if(this.message_list){var h,f,l,m=[],n=this.message_list.get_selection();h=0;for(len=n.length;h<len;h++)f=n[h],this.env.threading&&(e+=this.update_thread(f),l=this.message_list.find_root(f),l!=f&&0>$.inArray(l,m)&&m.push(l)),g&&this.message_list.remove_row(f,this.env.display_next&&h==n.length-1);
!this.env.display_next&&g&&this.message_list.clear_selection();h=0;for(len=m.length;h<len;h++)this.add_tree_icons(m[h])}0>e?b._count=-1*e:0<e&&g&&this.delete_excessive_thread_rows();g||(b._refresh=1);d||(d=this.display_message(this.get_label("move"==a?"movingmessage":"deletingmessage"),"loading"));this.http_post(a,b,d)};this.selection_post_data=function(a){"object"!=typeof a&&(a={});a._mbox=this.env.mailbox;if(!a._uid){var b=this.env.uid?[this.env.uid]:this.message_list.get_selection();a._uid=this.uids_to_list(b)}this.env.action&&
(a._from=this.env.action);this.env.search_request&&(a._search=this.env.search_request);this.env.display_next&&this.env.next_uid&&(a._next_uid=this.env.next_uid);return a};this.mark_message=function(a,b){var d=[],e=[],g,h,f,l=this.message_list;b?d[0]=b:this.env.uid?d[0]=this.env.uid:l&&(d=l.get_selection());if(l)for(l.focus(),h=0,g=d.length;h<g;h++)f=d[h],("read"==a&&l.rows[f].unread||"unread"==a&&!l.rows[f].unread||"delete"==a&&!l.rows[f].deleted||"undelete"==a&&l.rows[f].deleted||"flagged"==a&&!l.rows[f].flagged||
"unflagged"==a&&l.rows[f].flagged)&&e.push(f);else e=d;if(e.length||this.select_all_mode)switch(a){case "read":case "unread":this.toggle_read_status(a,e);break;case "delete":case "undelete":this.toggle_delete_status(e);break;case "flagged":case "unflagged":this.toggle_flagged_status(a,d)}};this.toggle_read_status=function(a,b){var d,e=b.length,g=this.selection_post_data({_uid:this.uids_to_list(b),_flag:a}),h=this.display_message(this.get_label("markingmessage"),"loading");for(d=0;d<e;d++)this.set_message(b[d],
"unread","unread"==a?!0:!1);this.http_post("mark",g,h)};this.toggle_flagged_status=function(a,b){var d,e=b.length,g=this.selection_post_data({_uid:this.uids_to_list(b),_flag:a}),h=this.display_message(this.get_label("markingmessage"),"loading");for(d=0;d<e;d++)this.set_message(b[d],"flagged","flagged"==a?!0:!1);this.http_post("mark",g,h)};this.toggle_delete_status=function(a){var b=a.length,d,e,g=!0,h=this.message_list?this.message_list.rows:{};if(1==b)return!this.message_list||h[a[0]]&&!h[a[0]].deleted?
this.flag_as_deleted(a):this.flag_as_undeleted(a),!0;for(d=0;d<b;d++)if(e=a[d],h[e]&&!h[e].deleted){g=!1;break}g?this.flag_as_undeleted(a):this.flag_as_deleted(a);return!0};this.flag_as_undeleted=function(a){var b,d=a.length,e=this.selection_post_data({_uid:this.uids_to_list(a),_flag:"undelete"}),g=this.display_message(this.get_label("markingmessage"),"loading");for(b=0;b<d;b++)this.set_message(a[b],"deleted",!1);this.http_post("mark",e,g)};this.flag_as_deleted=function(a){for(var b=[],d=this.selection_post_data({_uid:this.uids_to_list(a),
_flag:"delete"}),e=this.display_message(this.get_label("markingmessage"),"loading"),g=this.message_list?this.message_list.rows:{},h=0,f=0,l=a.length;f<l;f++)uid=a[f],g[uid]&&(g[uid].unread&&(b[b.length]=uid),this.env.skip_deleted?(h+=this.update_thread(uid),this.message_list.remove_row(uid,this.env.display_next&&f==this.message_list.selection.length-1)):this.set_message(uid,"deleted",!0));this.env.skip_deleted&&this.message_list&&(this.env.display_next||this.message_list.clear_selection(),0>h?d._count=
...
d.push(e),f=f.substr(f.indexOf(e)+e.length+1).replace(/^\s*,\s*/,"")});var e=0<d.length;f.mailvelope_keyring.validKeyForAddress(d).then(function(g){var h=[];$.each(g,function(a,b){!1===b&&(e=!1,h.push(a))});if(!e&&h.length)return f.show_popup_dialog(f.get_label("nopubkeyfor").replace("$email",h.join(", "))+"<p>"+f.get_label("searchpubkeyservers")+"</p>",f.get_label("encryptedsendialog"),[{text:f.get_label("search"),"class":"mainaction",click:function(){var a=$(this);f.mailvelope_search_pubkeys(h,
function(){a.dialog("close")})}},{text:f.get_label("cancel"),click:function(){$(this).dialog("close")}}]),!1;if(!e)return d.length||(alert(f.get_label("norecipientwarning")),$("[name='_to']").focus()),!1;var k=[],l=f.env.identities[$("[name='_from'] option:selected").val()];$.each(f.env.identities,function(a,b){k.push(b.email)});f.mailvelope_keyring.validKeyForAddress(k).then(function(e){valid_sender=null;$.each(e,function(a,b){if(!1!==b&&(valid_sender=a,valid_sender==l))return!1});if(!valid_sender&&
...
function(a){console.log(a)})},function(a){console.error(a)})},function(a){console.error(a)});return!1};this.mailvelope_display_container=function(a,b,d,e){mailvelope.createDisplayContainer(a,b,d,{showExternalContent:this.env.safemode}).then(function(){$(a).addClass("mailvelope").children().not("iframe").hide();f.hide_message(e);setTimeout(function(){$(window).resize()},10)},function(a){console.error(a);f.hide_message(e);f.display_message("Message decryption failed: "+a.message,"error")})};this.mailvelope_search_pubkeys=
function(a,b){var d=[],e=new PublicKey,g=f.display_message(f.get_label("loading"),"loading");$.each(a,function(a,b){var g=$.Deferred();e.search(b,function(a,d){null!==d?g.resolve([b]):g.resolve([b].concat(a))});d.push(g)});$.when.apply($,d).then(function(){var a=[],d=[];$.each(arguments,function(b,e){var g=e.shift();e.length?d=d.concat(e):a.push(g)});f.hide_message(g);b(!0);d.length&&f.mailvelope_key_import_dialog(d);a.length&&f.display_message(f.get_label("nopubkeyfor").replace("$email",a.join(", ")),
"warning")}).fail(function(){console.error("Pubkey lookup failed with",arguments);f.hide_message(g);f.display_message("pubkeysearcherror","error");b(!1)})};this.mailvelope_key_import_dialog=function(a){var b=$("<div>").addClass("listing mailvelopekeyimport");$.each(a,function(a,e){var g=$("<div>").addClass("key");e.revoked&&g.addClass("revoked");e.disabled&&g.addClass("disabled");e.expired&&g.addClass("expired");g.append($("<label>").addClass("keyid").text(f.get_label("keyid")));g.append($("<a>").text(e.keyid.substr(-8).toUpperCase()).attr("href",
e.info).attr("target","_blank").attr("tabindex","-1"));g.append($("<label>").addClass("keylen").text(f.get_label("keylength")));g.append($("<span>").text(e.keylen));e.expirationdate&&(g.append($("<label>").addClass("keyexpired").text(f.get_label("keyexpired"))),g.append($("<span>").text((new Date(1E3*e.expirationdate)).toDateString())));e.revoked&&g.append($("<span>").addClass("keyrevoked").text(f.get_label("keyrevoked")));var h=$("<ul>").addClass("uids");$.each(e.uids,function(a,b){var d=$("<li>").addClass("uid");
b.revoked&&d.addClass("revoked");b.disabled&&d.addClass("disabled");b.expired&&d.addClass("expired");h.append(d.text(b.uid))});g.append(h);g.append($("<input>").attr("type","button").attr("rel",e.keyid).attr("value",f.get_label("import")).addClass("button importkey").prop("disabled",e.revoked||e.disabled||e.expired));b.append(g)});f.show_popup_dialog($("<div>").append($("<p>").html(f.get_label("encryptpubkeysfound"))).append(b),f.get_label("importpubkeys"),[{text:f.get_label("close"),click:function(){$(this).dialog("close")}}]);
b.on("click","input.button.importkey",function(){var a=$(this),b=a.attr("rel"),g=new PublicKey,h=f.display_message(f.get_label("loading"),"loading");g.get(b,function(g,l){f.hide_message(h);l?f.display_message(f.get_label("keyservererror"),"error"):f.mailvelope_keyring.importPublicKey(g).then(function(g){"REJECTED"!==g&&(g=b.substr(-8).toUpperCase(),a.closest(".key").fadeOut(),f.display_message(f.get_label("keyimportsuccess").replace("$key",g),"confirmation"))},function(a){console.log(a)})})})};this.expunge_mailbox=
function(a){var b,d={_mbox:a};a==this.env.mailbox&&(b=this.set_busy(!0,"loading"),d._reload=1,this.env.search_request&&(d._search=this.env.search_request));this.http_post("expunge",d,b)};this.purge_mailbox=function(a){var b,d={_mbox:a};if(!confirm(this.get_label("purgefolderconfirm")))return!1;a==this.env.mailbox&&(b=this.set_busy(!0,"loading"),d._reload=1);this.http_post("purge",d,b)};this.purge_mailbox_test=function(){return this.env.exists&&(this.env.mailbox==this.env.trash_mailbox||this.env.mailbox==
this.env.junk_mailbox||this.env.mailbox.startsWith(this.env.trash_mailbox+this.env.delimiter)||this.env.mailbox.startsWith(this.env.junk_mailbox+this.env.delimiter))};this.login_user_keyup=function(a){var b=rcube_event.get_keycode(a),d=$("#rcmloginpwd");return 13==b&&d.length&&!d.val()?(d.focus(),rcube_event.cancel(a)):!0};this.open_compose_step=function(a){a=this.url("mail/compose",a);this.env.compose_extwin&&!this.env.extwin?this.open_window(a):(this.redirect(a),this.env.extwin&&window.resizeTo(Math.max(this.env.popup_width,
$(window).width()),$(window).height()+24))};this.init_messageform=function(){if(!this.gui_objects.messageform)return!1;var a,b,d=$("[name='_from']"),e=$("[name='_to']"),g=$("input[name='_subject']"),f=$("[name='_message']").get(0),k="1"==$("input[name='_is_html']").val(),l=["cc","bcc","replyto","followupto"],m,n=this.opener();n&&"compose"==n.env.action&&(setTimeout(function(){1<opener.history.length?opener.history.back():n.redirect(n.get_task_url("mail"))},100),this.env.opened_extwin=!0);0<this.env.autocomplete_threads&&
(m={threads:this.env.autocomplete_threads,sources:this.env.autocomplete_sources});this.init_address_input_events(e,m);for(a in l)this.init_address_input_events($("[name='_"+l[a]+"']"),m);k||(a=this.env.top_posting&&this.env.compose_mode?0:f.value.length,"select-one"==d.prop("type")&&this.change_identity(d[0]),this.set_caret_pos(f,a),a&&$(f).scrollTop(f.scrollHeight));this.env.save_localstorage&&this.compose_restore_dialog(0,k);""==e.val()?b=e:""==g.val()?b=g:f&&(b=f);$(b).filter(":visible").focus();
this.env.compose_focus_elem=document.activeElement;this.compose_field_hash(!0);this.auto_save_start()};this.compose_restore_dialog=function(a,b){var d,e,g,h=this.local_storage_get_item("compose.index",[]),k=function(a){++a<h.length&&f.compose_restore_dialog(a,b)};for(d=a||0;d<h.length;d++)if(e=h[d],g=this.local_storage_get_item("compose."+e,null,!0)){if(g.changed&&e==this.env.compose_id){this.restore_compose_form(e,b);break}if(!(this.env.draft_id&&g.draft_id&&g.draft_id!=this.env.draft_id||this.env.reply_msgid&&
g.reply_msgid!=this.env.reply_msgid)&&g.changed&&g.session!=this.env.session_id){this.show_popup_dialog(this.get_label("restoresavedcomposedata").replace("$date",(new Date(g.changed)).toLocaleString()).replace("$subject",g._subject).replace(/\n/g,"<br/>"),this.get_label("restoremessage"),[{text:this.get_label("restore"),"class":"mainaction",click:function(){f.restore_compose_form(e,b);f.remove_compose_data(e);f.save_compose_form_local();$(this).dialog("close")}},{text:this.get_label("delete"),"class":"delete",
click:function(){f.remove_compose_data(e);$(this).dialog("close");k(d)}},{text:this.get_label("ignore"),click:function(){$(this).dialog("close");k(d)}}]);break}}};this.init_address_input_events=function(a,b){this.env.recipients_delimiter=this.env.recipients_separator+" ";a.keydown(function(a){return f.ksearch_keydown(a,this,b)}).attr({autocomplete:"off","aria-autocomplete":"list","aria-expanded":"false",role:"combobox"})};this.submit_messageform=function(a,b){var d=this.gui_objects.messageform;if(d){if(!b&&
this.env.is_sent)return this.show_popup_dialog(this.get_label("messageissent"),"",[{text:this.get_label("save"),"class":"mainaction",click:function(){f.submit_messageform(!1,!0);$(this).dialog("close")}},{text:this.get_label("cancel"),click:function(){$(this).dialog("close")}}]);if(this.mailvelope_editor)return this.mailvelope_submit_messageform(a,b);var e=this.set_busy(!0,a||b?"savingmessage":"sendingmessage"),g=this.spellcheck_lang(),h=[];$("li",this.gui_objects.attachmentlist).each(function(){h.push(this.id.replace(/^rcmfile/,
""))});$('input[name="_attachments"]',d).val(h.join());d.target="savetarget";d._draft.value=a?"1":"";d.action=this.add_url(d.action,"_unlock",e);d.action=this.add_url(d.action,"_lang",g);d.action=this.add_url(d.action,"_framed",1);b&&(d.action=this.add_url(d.action,"_saveonly",1));this.submit_timer=setTimeout(function(){f.set_busy(!1,null,e);f.display_message(f.get_label("requesttimedout"),"error")},1E3*this.env.request_timeout);d.submit()}};this.compose_recipient_select=function(a){var b,d,e=0;for(d=
0;d<a.selection.length;d++)b=a.selection[d],this.env.contactdata[b]&&e++;this.enable_command("add-recipient",e)};this.compose_add_recipient=function(a){a||(a=$(this.env.focused_field).filter(":visible"),a=a.length?a.attr("id").replace("_",""):"to");var b=[],d=$("#_"+a),e=this.env.recipients_delimiter;if(this.contact_list&&this.contact_list.selection.length)for(var g,f=0;f<this.contact_list.selection.length;f++)if((g=this.contact_list.selection[f])&&this.env.contactdata[g]&&(b.push(this.env.contactdata[g]),
"E"==g.charAt(0)&&0>this.env.contactdata[g].indexOf("@")&&d.length)){var k=g.substr(1);this.group2expand[k]={name:this.env.contactdata[g],input:d.get(0)};this.http_request("group-expand",{_source:this.env.source,_gid:k},!1)}b.length&&d.length&&(g=d.val(),f=RegExp(RegExp.escape(e)+"\\s*$"),g&&!f.test(g)&&(g+=e+" "),d.val(g+b.join(e+" ")+e+" ").change(),this.triggerEvent("add-recipient",{field:a,recipients:b}));return b.length};this.check_compose_input=function(a){var b=$("[name='_to']"),d=$("[name='_cc']"),
e=$("[name='_bcc']"),g=$("[name='_from']"),h=$("[name='_subject']");if("text"==g.prop("type")&&!rcube_check_email(g.val(),!0))return alert(this.get_label("nosenderwarning")),g.focus(),!1;d=b.val()?b.val():d.val()?d.val():e.val();if(!rcube_check_email(d.replace(/^\s+/,"").replace(/[\s,;]+$/,""),!0))return alert(this.get_label("norecipientwarning")),b.focus(),!1;for(var k in this.env.attachments)if("object"===typeof this.env.attachments[k]&&!this.env.attachments[k].complete)return alert(this.get_label("notuploadedwarning")),
!1;if(""==h.val()){var b={},l=$('<div class="prompt">').html('<div class="message">'+this.get_label("nosubjectwarning")+"</div>").appendTo(document.body),m=$("<input>").attr({type:"text",size:30}).val(this.get_label("nosubject")).appendTo(l),n=function(){h.val(m.val());l.dialog("close");f.command(a,{nocheck:!0})};b[this.get_label("sendmessage")]=function(){n($(this))};b[this.get_label("cancel")]=function(){h.focus();$(this).dialog("close")};l.dialog({modal:!0,resizable:!1,buttons:b,close:function(a,
b){$(this).remove()}});m.select().keydown(function(a){13==a.which&&n()});return!1}if(!this.mailvelope_editor&&!this.editor.get_content()&&!confirm(this.get_label("nobodywarning")))return this.editor.focus(),!1;this.editor.save();return!0};this.toggle_editor=function(a,b,d){b=this.editor.toggle(a.html,a.noconvert||!1);a.mode=a.html?"html":"plain";!b&&d&&(a.mode=a.html?"plain":"html",$(d.target).filter("select").val(a.mode));b&&$("input[name='_is_html']").val(a.html?1:0);return b};this.insert_response=
function(a){a=this.env.textresponses[a]?this.env.textresponses[a].text:null;if(!a)return!1;this.editor.replace(a)};this.save_response=function(){var a={},b=this.editor.get_content({selection:!0,format:"text",nosig:!0}),d='<form class="propform"><div class="prop block"><label>'+this.get_label("responsename")+'</label><input type="text" name="name" id="ffresponsename" size="40" /></div><div class="prop block"><label>'+this.get_label("responsetext")+'</label><textarea name="text" id="ffresponsetext" cols="40" rows="8"></textarea></div></form>';
a[this.get_label("save")]=function(a){a=$("#ffresponsename").val();var b=$("#ffresponsetext").val();if(!b)return $("#ffresponsetext").select(),!1;a||(a=b.substring(0,40));var d=f.display_message(f.get_label("savingresponse"),"loading");f.http_post("settings/responses",{_insert:1,_name:a,_text:b},d);$(this).dialog("close")};a[this.get_label("cancel")]=function(){$(this).dialog("close")};this.show_popup_dialog(d,this.get_label("newresponse"),a,{button_classes:["mainaction"]});$("#ffresponsetext").val(b);
$("#ffresponsename").select()};this.add_response_item=function(a){var b=a.key;this.env.textresponses[b]=a;if(this.gui_objects.responseslist){var d=$("<li>").appendTo(this.gui_objects.responseslist);$("<a>").addClass("insertresponse active").attr("href","#").attr("rel",b).attr("tabindex","0").html(this.quote_html(a.name)).appendTo(d).mousedown(function(a){return rcube_event.cancel(a)}).on("mouseup keypress",function(a){if("mouseup"==a.type||13==rcube_event.get_keycode(a))return f.command("insert-response",
$(this).attr("rel")),$(document.body).trigger("mouseup"),rcube_event.cancel(a)})}};this.edit_responses=function(){};this.delete_response=function(a){!a&&this.responses_list&&(a=this.responses_list.get_selection()[0]);a&&confirm(this.get_label("deleteresponseconfirm"))&&this.http_post("settings/delete-response",{_key:a},!1)};this.spellcheck_state=function(){var a=this.editor.spellcheck_state();$.each(this.buttons.spellcheck||[],function(b,d){$("#"+d.id)[a?"addClass":"removeClass"]("selected")});return a};
this.spellcheck_lang=function(){return this.editor.get_language()};this.spellcheck_lang_set=function(a){this.editor.set_language(a)};this.spellcheck_resume=function(a){this.editor.spellcheck_resume(a)};this.set_draft_id=function(a){if(a&&a!=this.env.draft_id){var b={task:"mail",action:""};(b=this.opener(!1,b)||this.opener(!0,b))&&b.env.mailbox==this.env.drafts_mailbox&&b.command("checkmail");this.env.draft_id=a;$("input[name='_draft_saveid']").val(a);window.frames.savetarget&&window.frames.savetarget.history&&
!this.draft_autosave_submit&&!this.mailvelope_editor&&window.frames.savetarget.history.back();this.draft_autosave_submit=!1}this.remove_compose_data(this.env.compose_id);this.compose_skip_unsavedcheck=!1};this.auto_save_start=function(){this.env.draft_autosave&&(this.draft_autosave_submit=!1,this.save_timer=setTimeout(function(){f.draft_autosave_submit=!0;f.command("savedraft")},1E3*this.env.draft_autosave));!this.local_save_timer&&window.localStorage&&this.env.save_localstorage&&(this.compose_type_activity=
this.compose_type_activity_last=0,$(document).keypress(function(a){f.compose_type_activity++}),this.local_save_timer=setInterval(function(){f.compose_type_activity>f.compose_type_activity_last&&(f.save_compose_form_local(),f.compose_type_activity_last=f.compose_type_activity)},5E3),$(window).on("unload",function(){f.env.server_error||f.remove_compose_data(f.env.compose_id)}));window.onbeforeunload||(window.onbeforeunload=function(){if(!f.compose_skip_unsavedcheck&&f.cmp_hash!=f.compose_field_hash())return f.get_label("notsentwarning")});
this.busy=!1};this.compose_field_hash=function(a){var b,d,e,g="",f=["to","cc","bcc","subject"];for(b=0;b<f.length;b++)if(e=$('[name="_'+f[b]+'"]').val())g+=e+":";g+=this.editor.get_content({refresh:!1});if(this.env.attachments)for(d in this.env.attachments)g+=d;this.mailvelope_editor&&(g+=";"+(new Date).getTime());a&&(this.cmp_hash=g);return g};this.save_compose_form_local=function(){if(this.env.save_localstorage){var a={session:this.env.session_id,changed:(new Date).getTime()},b=!0;this.editor.save();
...
this.change_identity=function(a,b){if(!a||!a.options)return!1;b||(b=this.env.show_sig);var d=a.options[a.selectedIndex].value,e=this.env.identity,g=this.env.recipients_separator,h=RegExp.escape(g);this.env.signatures&&this.env.signatures[d]?(this.enable_command("insert-sig",!0),this.env.compose_commands.push("insert-sig")):this.enable_command("insert-sig",!1);if(!this.env.identities_initialized&&(this.env.identities_initialized=!0,this.env.show_sig_later&&(this.env.show_sig=!0),this.env.opened_extwin))return;
$.each(["replyto","bcc"],function(){var a,b=e&&f.env.identities[e]?f.env.identities[e][this]:"",m=d&&f.env.identities[d]?f.env.identities[d][this]:"",n=$('[name="_'+this+'"]'),p=n.val();b&&p&&(a=RegExp("\\s*"+RegExp.escape(b)+"\\s*"),p=p.replace(a,""));a=RegExp(h+"\\s*"+h,"g");p=String(p).replace(a,g);a=RegExp("^[\\s"+h+"]+");p=p.replace(a,"");m&&-1==p.indexOf(m)&&-1==p.indexOf(m.replace(/"/g,""))&&(p&&(a=RegExp("["+h+"\\s]+$"),p=p.replace(a,"")+g+" "),p+=m+g+" ");(b||m)&&n.val(p).change()});this.editor.change_signature(d,
...
else for(p in b.childs)s.push(p);for(k=0;k<s.length;k++)p=s[k],h=b.childs[p],h=$("<input>").addClass("ff_"+p).attr({type:"text",name:"_"+p+n,size:h.size}).appendTo(l),l.append(t[k]||" "),this.init_edit_field(p,h),q||(q=h);h=q}else if("select"==b.type){h=$("<select>").addClass("ff_"+a).attr({name:"_"+a+n,id:k}).appendTo(l);var v=h.attr("options");v[v.length]=new Option("---","");b.options&&$.each(b.options,function(a,b){v[v.length]=new Option(b,a)})}h&&($('<a href="#del"></a>').addClass("contactfieldbutton deletebutton").attr({title:this.get_label("delete"),
...
setTimeout(function(){f.hide_message(g,"loading"==b)},d),g;var h=$("<div>").addClass(b).html(a).data("key",e);$(this.gui_objects.message).append(h).show();this.messages[e]={obj:h,elements:[g]};"loading"==b?this.messages[e].labels=[{id:g,msg:a}]:"uploading"!=b&&h.click(function(){return f.hide_message(h)}).attr("role","alert");this.triggerEvent("message",{message:a,type:b,timeout:d,object:h});0<d&&setTimeout(function(){f.hide_message(g,"loading"!=b)},d);return g};this.hide_message=function(a,b){if(this.is_framed())return parent.rcmail.hide_message(a,
b);if(this.gui_objects.message){var d,e,g,f,k=this.messages;if("object"===typeof a)f=$(a),d=f.data("key"),this.hide_message_object(f,b),k[d]&&delete k[d];else for(d in k)for(e in k[d].elements)if(k[d]&&k[d].elements[e]==a)if(k[d].elements.splice(e,1),!k[d].elements.length)this.hide_message_object(k[d].obj,b),delete k[d];else if("loading"==d)for(g in k[d].labels)k[d].labels[g].id==a?delete k[d].labels[g]:(f=k[d].labels[g].msg,k[d].obj.html(f))}};this.hide_message_object=function(a,b){b?a.fadeOut(600,
...
"#";k.className="icon";$.each(this.env.mailboxes_list,function(){var a=0,b=0,d=f.env.mailboxes[this],h=d.id,q=$(k.cloneNode(!1)),s=$("<li>");d.virtual?q.addClass("virtual").attr("aria-disabled","true").attr("tabindex","-1"):q.addClass("active").data("id",d.id);for(d["class"]&&q.addClass(d["class"]);0<=(b=h.indexOf(g,b));)a++,b++;q.css("padding-left",a?16*a+"px":0);q.append($("<span>").text(d.name));s.append(q);e.push(s)});h.append(e).appendTo(d);d.css({left:"-1000px",top:"-1000px"}).appendTo($("body")).show();
10<e.length&&d.css("max-height",10*$("li",d)[0].offsetHeight+9);d.on("click","a.active",function(a){d.data("callback")($(this).data("id"));return!1});this.folder_selector_element=d}d.data("callback",b);this.show_menu("folder-selector",!0,a)};this.show_menu=function(a,b,d){var e="object"==typeof a?a.menu:a,g=$("#"+e),f=d&&d.target?$(d.target):$(g.attr("rel")||"#"+e+"link"),k=rcube_event.is_keyboard(d),l=g.attr("data-align")||"",m=!1;"A"!=f.get(0).tagName&&f.closest("a").length&&(f=f.closest("a"));
"string"==typeof a&&(a={menu:e});g.length||(g=this.triggerEvent("menu-get",{name:e,props:a,originalEvent:d}));if(!g||!g.length)return this.triggerEvent(!1===b?"menu-close":"menu-open",{name:e,props:a,originalEvent:d});g.appendTo(document.body);"undefined"==typeof b&&(b=g.is(":visible")?!1:!0);if(b&&f.length){var n=$(window),p=f.offset(),q=0<=l.indexOf("bottom"),m="menuitem"==f.attr("role")||0<f.closest("[role=menuitem]").length;f.offsetWidth=f.outerWidth();f.offsetHeight=f.outerHeight();!q&&p.top+
f.offsetHeight+g.height()>n.height()&&(q=!0);0<=l.indexOf("right")?p.left=p.left+f.outerWidth()-g.width():m&&(p.left=p.left+f.offsetWidth-5,p.top-=f.offsetHeight);p.left+g.width()>n.width()&&(p.left=n.width()-g.width()-12);p.top=Math.max(0,p.top+(q?-g.height():f.offsetHeight));g.css({left:p.left+"px",top:p.top+"px"})}if(b){for(l=this.menu_stack.length-1;m&&0<=l;l--)$(f).parents("#"+this.menu_stack[l]).length||"menuitem"==$(d.target).parent().attr("role")||this.hide_menu(this.menu_stack[l],d);m&&this.menu_stack.length?
(g.data("parent",$.last(this.menu_stack)),g.css("z-index",($("#"+$.last(this.menu_stack)).css("z-index")||0)+1)):!m&&this.menu_stack.length&&this.hide_menu(this.menu_stack[0],d);g.show().attr("aria-hidden","false").data("opener",f.attr("aria-expanded","true").get(0));this.triggerEvent("menu-open",{name:e,obj:g,props:a,originalEvent:d});this.menu_stack.push(e);if(this.menu_keyboard_active=b&&k)this.focused_menu=e,g.find("a,input:not(:disabled)").not("[aria-disabled=true]").first().focus()}else this.hide_menu(e,
...
e,g,d,a)}})};this.http_post=function(a,b,d){return this.http_request(a,b,d,"POST")};this.abort_request=function(a){a.request&&a.request.abort();a.lock&&this.set_busy(!1,null,a.lock)};this.http_response=function(a){if(a){a.unlock&&this.set_busy(!1);this.triggerEvent("responsebefore",{response:a});this.triggerEvent("responsebefore"+a.action,{response:a});a.env&&this.set_env(a.env);var b;if("object"===typeof a.texts)for(b in a.texts)"string"===typeof a.texts[b]&&this.add_label(b,a.texts[b]);a.exec&&
...
this.enable_command("delete","edit",d);this.enable_command("export",this.contact_list&&0<this.contact_list.rowcount);this.enable_command("export-selected","print",!1)}case "move":"show"==this.env.action?(this.enable_command(this.env.message_commands,!0),this.env.list_post||this.enable_command("reply-list",!1)):"addressbook"==this.task&&this.triggerEvent("listupdate",{folder:this.env.source,rowcount:this.contact_list.rowcount});case "purge":case "expunge":"mail"==this.task&&(this.env.exists||(this.env.contentframe&&
this.show_contentframe(!1),this.enable_command(this.env.message_commands,"purge","expunge","select-all","select-none","expand-all","expand-unread","collapse-all",!1)),this.message_list&&this.triggerEvent("listupdate",{folder:this.env.mailbox,rowcount:this.message_list.rowcount}));break;case "refresh":case "check-recent":$.each(this.env.recent_flags||{},function(a,b){f.set_message(a,"deleted",b.deleted);f.set_message(a,"replied",b.answered);f.set_message(a,"unread",!b.seen);f.set_message(a,"forwarded",
b.forwarded);f.set_message(a,"flagged",b.flagged)}),delete this.env.recent_flags;case "getunread":case "search":this.env.qsearch=null;case "list":if("mail"==this.task){d=this.is_multifolder_listing();var e=this.message_list;b=this.env.list_uid;this.enable_command("show","select-all","select-none",0<this.env.messagecount);this.enable_command("expunge",this.env.exists&&!d);this.enable_command("purge",this.purge_mailbox_test()&&!d);this.enable_command("import-messages",!d);this.enable_command("expand-all",
"expand-unread","collapse-all",this.env.threading&&this.env.messagecount&&!d);if(e){if("list"==a.action||"search"==a.action)b&&(e.rows[b]||(b+="-"+this.env.mailbox),e.rows[b]&&e.select(b),delete this.env.list_uid),this.enable_command("set-listmode",this.env.threads&&!d),0<e.rowcount&&!$(document.activeElement).is("input,textarea")&&e.focus(),e.triggerEvent("select");"getunread"!=a.action&&this.triggerEvent("listupdate",{folder:this.env.mailbox,rowcount:e.rowcount})}}else"addressbook"==this.task&&
(this.enable_command("export",this.contact_list&&0<this.contact_list.rowcount),"list"==a.action||"search"==a.action)&&(this.enable_command("search-create",""==this.env.source),this.enable_command("search-delete",this.env.search_id),this.update_group_commands(),0<this.contact_list.rowcount&&!$(document.activeElement).is("input,textarea")&&this.contact_list.focus(),this.triggerEvent("listupdate",{folder:this.env.source,rowcount:this.contact_list.rowcount}));break;case "list-contacts":case "search-contacts":this.contact_list&&
0<this.contact_list.rowcount&&this.contact_list.focus()}a.unlock&&this.hide_message(a.unlock);this.triggerEvent("responseafter",{response:a});this.triggerEvent("responseafter"+a.action,{response:a});this.start_keepalive()}};this.http_error=function(a,b,d,e,g){d=a.statusText;this.set_busy(!1,null,e);a.abort();this.unload||(a.status&&d?this.display_message(this.get_label("servererror")+" ("+d+")","error"):"timeout"==b?this.display_message(this.get_label("requesttimedout"),"error"):0==a.status&&"abort"!=
b&&this.display_message(this.get_label("connerror"),"error"),(b=a.getResponseHeader("Location"))&&"compose"!=this.env.action&&this.redirect(b),403==a.status?(this.is_framed()?parent:window).location.reload():"keep-alive"==g&&setTimeout(function(){f.keep_alive();f.start_keepalive()},3E4))};this.session_error=function(a){this.env.server_error=401;"compose"==this.env.action?(this.save_compose_form_local(),this.compose_skip_unsavedcheck=!0):a&&setTimeout(function(){f.redirect(a,!0)},2E3)};this.iframe_loaded=
function(a){this.set_busy(!1,null,a);this.submit_timer&&clearTimeout(this.submit_timer)};this.multi_thread_http_request=function(a){var b,d,e=(new Date).getTime(),f=a.threads||1;a.reqid=e;a.running=0;a.requests=[];a.result=[];a._items=$.extend([],a.items);a.lock||(a.lock=this.display_message(this.get_label("loading"),"loading"));this.http_request_jobs[e]=a;for(b=0;b<f;b++){d=a._items.shift();if(void 0===d)break;a.running++;a.requests.push(this.multi_thread_send_request(a,d))}return e};this.multi_thread_send_request=
function(a,b){var d,e,f;if(a.postdata){e={};for(d in a.postdata)e[d]=String(a.postdata[d]).replace("%s",b);e._reqid=a.reqid}else if("string"==typeof a.query)f=a.query.replace("%s",b),f+="&_reqid="+a.reqid;else if("object"==typeof a.query&&a.query){f={};for(d in a.query)f[d]=String(a.query[d]).replace("%s",b);f._reqid=a.reqid}return e?this.http_post(a.action,e):this.http_request(a.action,f)};this.multi_thread_http_response=function(a,b){var d=this.http_request_jobs[b];if(d&&!(0>=d.running||d.cancelled)){d.running--;
if(d.onresponse&&"function"==typeof d.onresponse)d.onresponse(a);d.result=$.extend(d.result,a);var e=d._items.shift();void 0!==e?(d.running++,d.requests.push(this.multi_thread_send_request(d,e))):0==d.running&&(d.whendone&&"function"==typeof d.whendone&&d.whendone(d.result),this.set_busy(!1,"",d.lock),delete this.http_request_jobs[b])}};this.multi_thread_request_abort=function(a){if(a=this.http_request_jobs[a]){for(var b=0;0<a.running&&b<a.requests.length;b++)a.requests[b].abort&&a.requests[b].abort();
a.running=0;a.cancelled=!0;this.set_busy(!1,"",a.lock)}};this.async_upload_form=function(a,b,d){var e=(new Date).getTime(),f="rcmupload"+e,h=this.async_upload_form_frame(f);if(this.env.upload_progress_name){var k=this.env.upload_progress_name,l=$("input[name="+k+"]",a);l.length||(l=$("<input>").attr({type:"hidden",name:k}),l.prependTo(a));l.val(e)}h.on("load",{ts:e},d);$(a).attr({target:f,action:this.url(b,{_id:this.env.compose_id||"",_uploadid:e,_from:this.env.action}),method:"POST"}).attr(a.encoding?
...
!1);var b,d=a.target.files||a.dataTransfer.files,e=window.FormData?new FormData:null,g=(this.env.filedrop.fieldname||"_file")+(this.env.filedrop.single?"":"[]"),h="------multipartformboundary"+(new Date).getTime(),k="--"+h+"\r\n",l={_id:this.env.compose_id||this.env.cid||"",_remote:1,_from:this.env.action};if(d&&d.length)for(var m=function(){var a=1<d.length,b=(new Date).getTime(),a=$("<span>").text(a?f.get_label("uploadingmany"):d[0].name).html();f.add2attachment_list(b,{name:"",html:a,classname:"uploading",
complete:!1})||(f.file_upload_id=f.set_busy(!0,"uploading"));k+="--"+h+"--\r\n";l._uploadid=b;$.ajax({type:"POST",dataType:"json",url:f.url(f.env.filedrop.action||"upload",l),contentType:e?!1:"multipart/form-data; boundary="+h,processData:!1,timeout:0,data:e||k,headers:{"X-Roundcube-Request":f.env.request_token},xhr:function(){var a=jQuery.ajaxSettings.xhr();!e&&a.sendAsBinary&&(a.send=a.sendAsBinary);return a},success:function(a){f.http_response(a)},error:function(a,b,d){f.http_error(a,b,d,null,
"attachment")}})},n=this.env.filedrop.single?0:d.length-1,p=b=0,q;b<=n&&(q=d[p]);p++){if(q.name||(q.name=q.fileName),q.size||(q.size=q.fileSize),q.type||(q.type="application/octet-stream"),!e&&/[^\x20-\x7E]/.test(q.name)&&(q.name_bin=unescape(encodeURIComponent(q.name))),!this.env.filedrop.filter||q.type.match(RegExp(this.env.filedrop.filter))){if(e){if(e.append(g,q),b==n)return m()}else if(window.FileReader){var s=new FileReader;s.onload=function(a,b){return function(d){k+='Content-Disposition: form-data; name="'+
g+'"';k+='; filename="'+(q.name_bin||a.name)+'"\r\n';k+="Content-Length: "+a.size+"\r\n";k+="Content-Type: "+a.type+"\r\n\r\n";k+=s.result+"\r\n";k+="--"+h+"\r\n";if(b==n)return m()}}(q,b);s.readAsBinaryString(q)}else if(q.getAsBinary&&(k+='Content-Disposition: form-data; name="'+g+'"',k+='; filename="'+(q.name_bin||q.name)+'"\r\n',k+="Content-Length: "+q.size+"\r\n",k+="Content-Type: "+q.type+"\r\n\r\n",k+=q.getAsBinary()+"\r\n",k+="--"+h+"\r\n",b==n))return m();b++}}else if(b=a.dataTransfer.getData("roundcube-uri"))p=
(new Date).getTime(),a=$("<span>").text(a.dataTransfer.getData("roundcube-name")||this.get_label("attaching")).html(),l._uri=b,l._uploadid=p,this.add2attachment_list(p,{name:"",html:a,classname:"uploading",complete:!1})||(this.file_upload_id=this.set_busy(!0,"attaching")),this.http_post(this.env.filedrop.action||"upload",l)};this.start_keepalive=function(){if(this.env.session_lifetime&&!this.env.framed&&!this.env.extwin&&"login"!=this.task&&"print"!=this.env.action){this._keepalive&&clearInterval(this._keepalive);
var a=500*Math.min(1800,this.env.session_lifetime);this._keepalive=setInterval(function(){f.keep_alive()},3E4>a?3E4:a)}};this.start_refresh=function(){!this.env.refresh_interval||this.env.framed||this.env.extwin||"login"==this.task||"print"==this.env.action||(this._refresh&&clearInterval(this._refresh),this._refresh=setInterval(function(){f.refresh()},1E3*this.env.refresh_interval))};this.keep_alive=function(){this.busy||this.http_request("keep-alive")};this.refresh=function(){if(this.busy)setTimeout(function(){f.refresh();
...
[]);d=0;for(e=a.elements.length;d<e;d++)if(f=a.elements[d],"hidden"!=f.type)if(b&&f.disabled)this.disabled_form_elements.push(f);else if(b||0>$.inArray(f,this.disabled_form_elements))f.disabled=b}};this.mailto_handler_uri=function(){return location.href.split("?")[0]+"?_task=mail&_action=compose&_to=%s"};this.register_protocol_handler=function(a){try{window.navigator.registerProtocolHandler("mailto",this.mailto_handler_uri(),a)}catch(b){this.display_message(String(b),"error")}};this.check_protocol_handler=
function(a,b){var d=window.navigator;d&&"function"==typeof d.registerProtocolHandler?"function"==typeof d.isProtocolHandlerRegistered?(d=d.isProtocolHandlerRegistered("mailto",this.mailto_handler_uri()))&&$(b).parent().find(".mailtoprotohandler-status").html(d):$(b).click(function(){f.register_protocol_handler(a);return!1}):$(b).addClass("disabled").click(function(){return!1})};this.browser_capabilities_check=function(){this.env.browser_capabilities||(this.env.browser_capabilities={});$.each(["pdf",
"flash","tif"],function(){void 0===f.env.browser_capabilities[this]&&(f.env.browser_capabilities[this]=f[this+"_support_check"]())})};this.browser_capabilities=function(){if(!this.env.browser_capabilities)return"";var a,b=[];for(a in this.env.browser_capabilities)b.push(a+"="+this.env.browser_capabilities[a]);return b.join()};this.tif_support_check=function(){window.setTimeout(function(){var a=new Image;a.onload=function(){f.env.browser_capabilities.tif=1};a.onerror=function(){f.env.browser_capabilities.tif=
0};a.src=f.assets_path("program/resources/blank.tif")},10);return 0};this.pdf_support_check=function(){var a=navigator.mimeTypes?navigator.mimeTypes["application/pdf"]:{},b=navigator.plugins,d=b.length,e=/Adobe Reader|PDF|Acrobat/i;if(a&&a.enabledPlugin)return 1;if("ActiveXObject"in window){try{if(a=new ActiveXObject("AcroPDF.PDF"))return 1}catch(g){}try{if(a=new ActiveXObject("PDF.PdfCtrl"))return 1}catch(h){}}for(i=0;i<d;i++)if(a=b[i],"String"===typeof a){if(e.test(a))return 1}else if(a.name&&e.test(a.name))return 1;
window.setTimeout(function(){$("<object>").css({position:"absolute",left:"-10000px"}).attr({data:f.assets_path("program/resources/dummy.pdf"),width:1,height:1,type:"application/pdf"}).load(function(){f.env.browser_capabilities.pdf=1}).error(function(){f.env.browser_capabilities.pdf=0}).appendTo($("body"))},10);return 0};this.flash_support_check=function(){var a=navigator.mimeTypes?navigator.mimeTypes["application/x-shockwave-flash"]:{};if(a&&a.enabledPlugin)return 1;if("ActiveXObject"in window)try{if(new ActiveXObject("ShockwaveFlash.ShockwaveFlash"))return 1}catch(b){}return 0};
this.assets_path=function(a){this.env.assets_path&&!a.startsWith(this.env.assets_path)&&(a=this.env.assets_path+a);return a};this.set_cookie=function(a,b,d){setCookie(a,b,d,this.env.cookie_path,this.env.cookie_domain,this.env.cookie_secure)};this.get_local_storage_prefix=function(){this.local_storage_prefix||(this.local_storage_prefix="roundcube."+(this.env.user_id||"anonymous")+".");return this.local_storage_prefix};this.local_storage_get_item=function(a,b,d){var e,f;try{e=localStorage.getItem(this.get_local_storage_prefix()+
a),f=JSON.parse(e)}catch(h){}return f||b||null};this.local_storage_set_item=function(a,b,d){try{return localStorage.setItem(this.get_local_storage_prefix()+a,JSON.stringify(b)),!0}catch(e){return!1}};this.local_storage_remove_item=function(a){try{return localStorage.removeItem(this.get_local_storage_prefix()+a),!0}catch(b){return!1}};this.print_dialog=function(){bw.safari?setTimeout("window.print()",10):window.print()}}
rcube_webmail.long_subject_title=function(f,u){if(!f.title){var a=$(f);a.width()+15*(u||0)>a.parent().width()&&(f.title=rcube_webmail.subject_text(f))}};rcube_webmail.long_subject_title_ex=function(f){if(!f.title){var u=$(f),a=$.trim(u.text()),a=$("<span>").text(a).css({position:"absolute","float":"left",visibility:"hidden","font-size":u.css("font-size"),"font-weight":u.css("font-weight")}).appendTo($("body")),b=a.width();a.remove();b+15*$("span.branch",u).width()>u.width()&&(f.title=rcube_webmail.subject_text(f))}};
rcube_webmail.subject_text=function(f){f=$(f).clone();f.find(".skip-on-drag").remove();return f.text()};rcube_webmail.set_iframe_events=function(f){$("iframe").each(function(){var u=$(this);$.each(f,function(a,b){u.on("load",function(d){try{$(this).contents().on(a,b)}catch(f){}});try{u.contents().on(a,b)}catch(d){}})})};rcube_webmail.prototype.get_cookie=getCookie;rcube_webmail.prototype.addEventListener=rcube_event_engine.prototype.addEventListener;rcube_webmail.prototype.removeEventListener=rcube_event_engine.prototype.removeEventListener;
