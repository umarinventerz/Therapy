	  ...
	  //ventana.close();  //cerramos la ventana
	   ...
	   function imprimirTerapiasSeleccionadas(){
        ...
        $('input:checkbox:checked').map(function() {
                    if(flag == 0 && this.name.substring(0,4) == 'ext_'){
                        valores_check += '**'
                        flag = 1;
                    }
                    if(this.value != 'on') {
                ...
                var ventana=window.open('../../controlador/payroll/print_teraphist.php?identificadores_check='+valores_check+'&date_start='+$("#date_start").val()+'&date_end='+$("#date_end").val()+'&type_employee='+$("#type_employee").val()+'&selectedTreatments='+$("#selectedTreatments").val(),'print_teraphist','width=1200, height=800, scrollbars=1, location=no,left=300,top=50');                              
            //ventana.document.write(objeto.innerHTML);  //imprimimos el HTML del objeto en la nueva ventana
            ventana.document.close();  //cerramos el documento
            ventana.print();  //imprimimos la ventana
            //ventana.close();
...
function number_format (number, decimals, decPoint, thousandsSep) {   
  ...
  return s.join(dec)
    ...
    function Notificar_Tarea(tarea,id_tareas) {
        ...
        new PNotify({
            title: '<center><b>NEW TASK</b></center>',
            text: tarea+'<center><button class="btn btn-primary" onclick="consultar_tareas('+id_tareas+');">Detalle</button>',
             hide:false,
            type: 'info'
        });
    ...
    function consultar_tareas(id_tareas){
        
        window.location.href = '../../vista/tareas/consultar_tareas.php?&id_tareas='+id_tareas;
