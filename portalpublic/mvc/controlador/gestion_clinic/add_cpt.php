...
$resultado_one = ejecutar($insert_note_cpt_relation,$conexion);
...
$id_diagnostic=$_POST['id_diagnosis'];


$sql_diagnostic1="SELECT * FROM diagnosiscodes
                    WHERE  DiagCodeId =".$id_diagnostic;

$resultado_diagnostic = ejecutar($sql_diagnostic1,$conexion);
while($row = mysqli_fetch_array($resultado_diagnostic)) {
    $diagnostic['code']=$row["DiagCodeValue"];
}
