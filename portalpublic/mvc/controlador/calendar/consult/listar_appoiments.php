...
if($own ==null){
    ...
    if($user_vacio=='si' && $patient_vacio=='si'){
         ...
         $sql="select C.*,C.id as id_calendar,CP.*,CP.id as id_date,CP.start as inicio,CP.end as fin,concat(P.Last_name,', ',P.First_name) as title,concat(E.Last_name,', ',E.First_name) as employee,E.discipline_id,D.Name as disciplina_name,T.color as color,L.name as locations,CR.*  FROM calendar_appointments C
             left join calendar_appoiment_date CP on(C.id=CP.calendar_appoiment_id) left join calendar_appoiment_recurring CR ON(CP.id=CR.calendar_date) left join patients P on(C.Pat_id=P.id) left join employee E on (C.therapist_id=E.id) left join discipline D ON(E.discipline_id=D.DisciplineId) left join tbl_type_appoiments T on (C.type=T.id) left join tbl_location_appoiments L on (C.location=L.id)  WHERE CP.start BETWEEN '".$inicio."' AND '".$fin."' ORDER BY CP.start ASC";
    }
    if($user_vacio !='si' && $patient_vacio=='si'){
