...
if(!isset($_SESSION['user_id'])){
    ...
    if($type_appoiments==1){
            ...
            if($conflicto=='si'){
                    ...
                    $insert = "INSERT into calendar_appointments (Pat_id,therapist_id,subject,location,date,recurring,next_weeks,biweekly,type,start,end,attendance,user_id)
                                    values ('".$pat_id."','".$therapista_id."','".$subject."','".$location."','".$date."','".$recurring."','".$next_weeks."','".$biweekly."', '".$type."','".$star_time."','".$end_time."','".$attendance."','".$_SESSION['user_id']."');";
                                    $resultado = ejecutar($insert,$conexion);
    ...
    if($type_appoiments==2){
        ...
        if($conflicto=='no'){
            ...
            $insert = "INSERT into calendar_appointments (Pat_id,therapist_id,subject,location,date,type,attendance,user_id,quantity,period,type_appoiment)
                        values ('".$pat_id."','".$therapista_id."','".$subject."','".$location."','".$date."','".$type."','".$attendance."','".$_SESSION['user_id']."','".$quantity."','".$period."','recurring');";
                        $resultado = ejecutar($insert,$conexion);
