...
if(isset($id) && count($id)>0){
    ...
    if($Without=='si'){
        for($i=0;$i<count($id);$i++){
            ...
            $enviar=$send_phone->send_sms_interfaz_without($id[$i],$subject,$text_date_change,$type_person,$variables,$data_autorizacion);
            if($enviar=='si'){
                $si=$si+1;
            }else{
                $no=$no+1;
        ...
        for($i=0;$i<count($id);$i++){
            ...
            $enviar=$send_phone->send_sms_interfaz($id[$i],$subject,$sms,$type_person,$id_templates,$data_autorizacion);
            if($enviar=='si'){
                $si=$si+1;
            }else{
                $no=$no+1;
...
?>
    <script>
        ...
        window.location="../../vista/report_mail/sendmsj_RingCentral.php";
    </script>
<?php   
}else{    
?>
