...
while($datos = mysqli_fetch_assoc($resultado)) {            
    ...
    $i++;
        ...
        //ACA FALTA EL CALCULO DEL WRITE_OFF
                    ...
                    if(isset($_POST["denied"]) && $_POST["denied"] == 'Denied'){ $denied = 1; } else { $denied = 0; }
                    if(isset($_POST["notes"]) && $_POST["notes"] != null){ $notes = $_POST["notes"]; } else { $notes = 'null'; }
                    
                    $update =" UPDATE tbl_treatments_charges SET pat_paid = '$".$patient_paid."', ins_paid = '$".$insurance_paid."', denied = '".$denied."', pat_balance = '$".$pat_balance."', ins_balance = '$".$ins_balance."', balance = '$".$balance."' ".$where_t;
