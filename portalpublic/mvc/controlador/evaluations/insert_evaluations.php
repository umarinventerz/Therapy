...
date_signed, id_prescription, patient_id,company, units, minute
s".$field.",therapist_signed,therapist_signed_date,therapist_signed_location_latitude,therapist_signed_location_longitude,therapist_signature) 
...
##############seccion que se encarga de insertar los treatmente en la parte que le corresponde
 ...
 $info_pat="SELECT * FROM discipline
                    WHERE  DisciplineId =".$_POST['discipline_id_eval_hidden'];
 ...
 $info_pat="SELECT * FROM diagnosiscodes
                    WHERE  DiagCodeId =".$_POST['diagnostic_eval_hidden'];
 $info_pat_id = ejecutar($info_pat,$conexion);
 while ($row=mysqli_fetch_array($info_pat_id)) {
     $campo_16 = $row['DiagCodeValue'];
 }


 $campo_17=$_POST['unid_of_eval'];
 $campo_15=$_POST['min_of_eval'];  #durations
 $campo_11=$_POST['cpt_h'];;#CPT_code
 $cpt=$campo_11;
 ...
 ####################esto me asegura cuando se editen las evaluations poder editar el reporte en especifico

 $insert_in_tbl_treatment_evaluations="INSERT INTO tbl_treatments_evaluations ( id_evaluations,id_treatments)
values ('". $id_tbl_evaluation."','".$id_tbl_treatments."');";
 $resultado = ejecutar($insert_in_tbl_treatment_evaluations,$conexion);
