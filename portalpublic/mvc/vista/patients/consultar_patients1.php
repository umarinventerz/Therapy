...
<form id="form_consultar_patients"  onSubmit="return validar_formulario(this);">      
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-8" align="left"><h3><font color="#BDBDBD">Consult Patients</font></h3>   </div>
...
<?php
    ...
    <div class="col-sm-3"><select id="Last_name_First_name" name="Last_name_First_name"><option value="">--- SELECT ---</option>
<?php
                                          //modifique aqui Cesar
                                 $sql  = "select * from patients where id is not null group by last_name";
                                 $conexion = conectar();
                                                            $resultado = ejecutar($sql,$conexion);
                                                            while ($row=  mysqli_fetch_assoc($resultado)){                                                                                                   
                print("<option value='".utf8_decode($row["Last_name"])."' >".utf8_decode($row["Last_name"])." </option>");
                                                            }
...
<div class="col-sm-3"><input type="checkbox" id="active" name="active" <?php if(isset($_GET['active']) && $_GET['active'] == '1') { echo 'checked'; }?>></div>   
        ...
        <div class="col-sm-3"><select id="Sex" name="Sex"><option value="">--- SELECT ---</option><option value="M">Male</option><option value="F">Female</option></select></div>
        </div>
...
<div class="col-sm-3"><input type="text" class="form-control" id="Phone" name="Phone" placeholder="Phone" onkeyup="Mayusculas(event, this)" value="<?php if(isset($_GET['Phone'])) { echo str_replace('|',' ',$_GET['Phone']); }?>"></div>
 ...
 <?php
                                                           //modifique aqui Cesar
                                                            $sql  = "SELECT * FROM physician p join patients pa on pa.PCP=p.Phy_id";
                                                            $conexion = conectar();
                                                            $resultado = ejecutar($sql,$conexion);
                                                            while ($row=  mysqli_fetch_assoc($resultado)){                                                                      
...
<div class="col-sm-3"><input type="text" class="form-control" id="Phy_NPI" name="Phy_NPI" placeholder="PCP NPI" onkeyup="Mayusculas(event, this)" value="<?php if(isset($_GET['Phy_NPI'])) { echo str_replace('|',' ',$_GET['Phy_NPI']); }?>"></div>
        ...
        <div class="col-sm-3"><select id="Ref_Physician" name="Ref_Physician"><option value="">--- SELECT ---</option>
             <?php
                                                           //modifique aqui Cesar
                                                            $sql  = "SELECT * FROM physician p join patients pa on pa.Ref_Physician=p.Phy_id";
                                                            $conexion = conectar();
                                                            $resultado = ejecutar($sql,$conexion);
                                                            while ($row=  mysqli_fetch_assoc($resultado)){                                                                      
                                                                            print("<option value='".utf8_decode($row["Phy_id"])."' >".utf8_decode($row["Name"])." </option>");
...
<div class="col-sm-3"><select id="Pri_Ins" name="Pri_Ins"><option value="">--- SELECT ---</option>
 <?php
                                                           //modifique aqui Cesar
                                                            $sql  = "SELECT * FROM seguros s join patients pa on pa.Pri_Ins=s.ID";
                                                            $conexion = conectar();
                                                            $resultado = ejecutar($sql,$conexion);
                                                            while ($row=  mysqli_fetch_assoc($resultado)){                                                                      
...
<?php
                                                           //modifique aqui Cesar
                                                            $sql  = "SELECT * FROM seguros s join patients pa on pa.Sec_INS=s.ID";
                                                            $conexion = conectar();
                                                            $resultado = ejecutar($sql,$conexion);
                                                            while ($row=  mysqli_fetch_assoc($resultado)){                                                                      
                                                                            print("<option value='".utf8_decode($row["ID"])."' >".utf8_decode($row["insurance"])." </option>");
...
<?php
                                                           //modifique aqui Cesar
                                                            $sql  = "SELECT * FROM seguros s join patients pa on pa.Ter_Ins=s.ID";
                                                            $conexion = conectar();
                                                            $resultado = ejecutar($sql,$conexion);
                                                            while ($row=  mysqli_fetch_assoc($resultado)){                                                                      
                                                                            print("<option value='".utf8_decode($row["ID"])."' >".utf8_decode($row["insurance"])." </option>");
...
<div class="col-sm-3"><input type="text" class="form-control" id="Cell" name="Cell" placeholder="Cell" onkeyup="Mayusculas(event, this)" value="<?php if(isset($_GET['Cell'])) { echo str_replace('|',' ',$_GET['Cell']); }?>"></div>           
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-10" align="left"> <button type="submit" class="btn btn-primary text-left">Consult</button> </div>
        </div>
</form>      
        <script>
