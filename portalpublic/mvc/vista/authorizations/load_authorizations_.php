...
if($_POST['insert'] == " Add "){
	...
	if($flag == 0){
		...
		if($row_cnt > 0){
			...
			values ('".$patient_name."','".$patient_id."','".$_POST['discipline']."','".$_POST['cpt']."','".$_POST['insurance']."','".$_POST['auth_number']."','".$_POST['auth_start']."','".$_POST['auth_thru']."','".$_POST['received']."','".$_POST['company']."','".$route_file_var."',1);";
			ejecutar($insert,$conexion);
                        ...
                        $sql_insert_document = "INSERT INTO tbl_documents (`id_type_document`,`id_type_person`,`id_table_relation`,`id_person`,`route_document`) "
                                . " VALUES ('2',1,".$id_tbl_authorizations.",'".$patient_id."','".$route_file_var."')";
		...
		<th>File Route</th>				            
					...
					while($row = mysqli_fetch_array($result01,MYSQLI_ASSOC)){
						...
						echo '<tr>	 						
							...
							<td>'.$status.'</td>
							<td><a onclick="window.open(\'../../../'.$row['route_file'].'\',\'\',\'width=900px,height=700px,noresize\');">'.$row['route_file'].'</a></td>
