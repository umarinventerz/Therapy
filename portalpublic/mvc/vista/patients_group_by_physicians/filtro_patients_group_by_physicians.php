...
<head>
    ...
    <script type="text/javascript" language="javascript">
       ...
       function validar_formulario(){
                var parametros_formulario = $('#form_patients_group_by_physicians').serialize();
                     
                $("#resultado").empty().html('<img src="../../../images/loader.gif" width="30" height="30"/>');
                $("#resultado").load("../patients_group_by_physicians/grafico_patients_group_by_physicians.php?"+parametros_formulario);
...
<form id="form_patients_group_by_physicians"  onSubmit="return validar_formulario(this);">              
                <div class="row">
                    <div class="col-lg-12">                        
                        <h3 class="page-header">Patients Group By Physicians</h3>                        
                    </div>
                </div>   
...
<option value="Column3D">Barra 3D  &nbsp;&nbsp;&nbsp; </option>
  ...
  <div class="form-group row">
        ...
        <div class="form-group row">
                             <label class="col-sm-2 form-control-label text-right"><font color="#585858">PHYSICIAN</font></label>
                        <div class="col-sm-8">
                           
                                    <select name='pcp' id='pcp' class='form-control' onchange="validar_formulario()">
                                            <option value=''>--- ALL ---</option>                
                                        <?php
                                        $sql  = "SELECT PCP , ROUND((COUNT(*) / (SELECT COUNT(*) FROM patients WHERE active='1' and PCP!='')) * 100,1) as cantidad,PCP as valor FROM patients  
                                         WHERE active='1' and pcp!='' GROUP BY valor order by cantidad desc";
                                        $conexion = conectar();
                                        $resultado = ejecutar($sql,$conexion);
                                        while ($row=  mysqli_fetch_assoc($resultado)){                                                                      
                                        print("<option value='".$row["PCP"]."' >".utf8_decode($row["PCP"])." </option>");
                                                            }
                                        ?>
                                    </select>     
