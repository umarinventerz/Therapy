...
.swal-wide{
    ...
    <script>	
    	...
    	function updateNew_Report_eval_done(pat_id,discipline,company,patient_name,cpt,type){	
		var eval_done;
		...
		$("#result_u"+pat_id).load("../../controlador/nuevos_reportes/new_report_update.php","&Patient_id="+pat_id+"&Discipline="+discipline+"&Company="+company+"&evalschedule="+$("#eval_schedule_"+pat_id).val()+"&evalDone="+eval_done+"&patient_name="+patient_name);
	...
	function updatePrescriptions(pat_id,discipline,company,patient_name,cpt,type){	
		...
		//alert($("#eval_schedule_"+pat_id).val());
		$("#result_u"+pat_id).load("../../controlador/prescriptions/update_prescriptions.php","&pat_id="+pat_id+"&Discipline="+discipline+"&Company="+company+"&evalschedule="+$("#eval_schedule_"+pat_id).val()+"&evalDone="+eval_done+"&patient_name="+patient_name);
		alert("Update Succefull");
		window.location="windows_report_amount.php?discipline="+discipline+"&cpt="+cpt+"&type="+type;
	}
	...
	function updateAllReadyTreatments(pat_id,discipline,company,patient_name,cpt,type){	
		...
		//alert($("#eval_schedule_"+pat_id).val());
		$("#result_all"+pat_id).load("../../controlador/prescriptions/update_prescriptions.php","&pat_id="+pat_id+"&Discipline="+discipline+"&Company="+company+"&allReadyTreatments="+all_ready_treatments+"&patient_name="+patient_name+"&type="+type);
		alert("Update Succefull");
		window.location="windows_report_amount.php?discipline="+discipline+"&cpt="+cpt+"&type="+type;
	}
...
$dayswaiting = '';
    ...
    <table class="table table-striped">
		...
		$while_reportes = "SELECT * FROM tbl_type_report 
                						WHERE  field_patient_copy IS NOT NULL 
                							AND name='".$type."'
                							order by `order`";
       ...
       $result_while_reportes = ejecutar($while_reportes,$conexion);                
        ...
        while($datos_reportes = mysqli_fetch_assoc($result_while_reportes)) {
                        ...
                        $query_count= "SELECT *, CONCAT(trim(patients.Last_name),',',trim(patients.First_name)) as Patient_name 
                        , TIMESTAMPDIFF(YEAR,patients.DOB,CURDATE()) AS AGE
                        ,patients.PCP as physician
                        ,patients.Pri_Ins as Insurance_name

                        FROM patients 
                        JOIN patients_copy ON patients.id=patients_copy.id
                        AND  patients.active=1
                        LEFT join prescription c on c.Patient_ID=patients.Pat_id
						and status=1
						and c.discipline='".$discipline."'
                        WHERE ".$datos_reportes['field_patient_copy'].$discipline."='1'
                        order by Patient_name asc;";
                        
                       $result2 = mysqli_query($conexion, $query_count);				                       
					 ...
					 echo  	'<td>'.$row['STATUS'].'</td>' 	;	
					  ...
					  if($_GET['type'] == 'NEED EVALUTATION'){
					   ...
					   if($_GET['type'] == 'READY FOR TREATMENT'){
						...
						echo '<td><input type="checkbox" name="all_ready_treatments_'.trim($row['Pat_id']).'" id="all_ready_treatments_'.trim($row['Pat_id']).'"></td>
						<td><div id="result_all'.trim($row['Pat_id']).'"><button type="button" class="btn btn-info btn-lg" onclick="updateAllReadyTreatments(\''.trim($row['Pat_id']).'\',\''.trim($_GET['discipline']).'\',\''.$row['Company'].'\',\''.$row['Patient_name'].'\',\''.$_GET['cpt'].'\',\''.$_GET['type'].'\');">Update</button></div></td>'; }
					   }
					   
					   if($_GET['type'] == 'POC EXPIRED NO PRESCRIPTION' || $_GET['type'] == 'POC EXPIRED THAT NEED ASK FOR AUTHORIZATION FOR EVAL' || $_GET['type'] == 'NOT SIGNED BY DOCTOR YET' || $_GET['type'] == 'ASK FOR AUTHORIZATION FOR TX' || $_GET['type'] == 'PROGRESS NOTES ADULTS' || $_GET['type'] == 'PROGRESS NOTES PEDRIATICS' || $_GET['type'] == 'DISCHARGE'){						
