...
if(isset($_GET['modal_eval_jorgito'])) {
                                                        
    $id_presc = $_GET['id_prescr'];
                                                                    
                                                                                       
    $appoiment123="SELECT E.*,concat(P.Last_name,', ',P.First_name) as patient,P.Pat_id,P.DOB,concat(P.PCP,', ',P.PCP_NPI) AS physician,C.*,concat(U.Last_name,', ',U.First_name) as employee,
                ...
                LEFT JOIN employee em ON V.user_id=em.id
                LEFT JOIN patients P ON(PR.Patient_ID=P.Pat_id)
                LEFT JOIN companies C ON(PR.Table_name=C.company_name) 
                LEFT JOIN diagnosiscodes D ON(PR.Diagnostic=D.DiagCodeValue)
                LEFT JOIN discipline DI ON(PR.Discipline=DI.Name)
                LEFT JOIN user_system U ON(U.user_id=em.id)
                LEFT JOIN tbl_visits V ON(V.app_id=PR.id) 
                  WHERE PR.id_prescription='". $id_presc."' ";

    $resultados = ejecutar($appoiment123,$conexion);
...
//Conculto las notas desde tbl_notes_documentation
   ...
   if(isset($_GET['id_note'])){
      ...
      $sql_notas="SELECT N.*,concat(P.Last_name,', ',P.First_name) as patient,P.Pat_id,P.DOB,E.npi_number as NPI ,C.*,concat(E.last_name,', ',E.first_name) as employee,
                ...
                LEFT JOIN discipline DI ON(CA.Discipline=DI.Name)
...
if(isset($_GET['modal_note_jorgito']))
{

    $id_poc=$_GET['id_poc'];


    $sql_notas="SELECT CA.*,concat(P.Last_name,', ',P.First_name) as patient,P.Pat_id,P.id as patient_id,P.DOB,E.npi_number as NPI ,C.*,concat(E.last_name,', ',E.first_name) as employee,
                DI.Name as disciplina,DI.DisciplineId as disciplineId, CA.Discipline as discipline_poc,CA.Company as company_poc,
                CA.diagnostic as diagnostic_poc,CA.id_careplans,  
                CONCAT(P.PCP, ' , NPI ', P.PCP_NPI )    as ppcp ,V.visit_date,
                CONCAT(P.dob , ' / ', TIMESTAMPDIFF( YEAR, P.dob, now() ) ,' Yrs.,  ' 
                      , TIMESTAMPDIFF( MONTH, P.dob, now()) % 12,' Months, ' 
                ...
                LEFT JOIN patients P ON(CA.Patient_ID=P.Pat_id)
                LEFT JOIN companies C ON(CA.Company=C.id) 
                LEFT JOIN discipline DI ON(CA.Discipline=DI.Name)

                LEFT JOIN tbl_evaluations TE ON(TE.id=CA.evaluations_id) 
                LEFT JOIN employee E ON(E.id=TE.id_user)
                LEFT JOIN tbl_visits V ON(TE.visit_id=V.id)
            WHERE CA.id_careplans='".$id_poc."'";
    ...
    $note[0]['discipline_id']=$note[0]['disciplineId'];
...
<script>
        ...
        $(document).ready(function(){
            ...
            var note_insert='<?=$_GET['note_insert']?>';

            if(note_insert==='si'){
                $("#modalAddNote").modal('show');
                        }
                    
            var eval_insert='<?=$_GET['eval_insert']?>';
