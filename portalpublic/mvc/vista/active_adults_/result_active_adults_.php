               ...
               if(!isset($_SESSION['user_id'])){
                ...
                $query_usuario = "SELECT * FROM (select  count(distinct patient_id) as expires,    week(careplans.poc_due) as week   from careplans  join patients on careplans.patient_id=patients.pat_id  					and patients.active=1  where  careplans.poc_due>=date(now()) and timestampdiff(year,patients.dob,curdate())  >= 18  group by week(careplans.poc_due) order by  careplans.poc_due asc limit 12 ) as t WHERE t.week = '$valor'";
