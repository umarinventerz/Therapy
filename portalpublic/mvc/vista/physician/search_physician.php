...
list($Phy_id,$company) = explode('-',$_POST['name']); 
...
echo $sql1="SELECT Phy_id,Name,NPI,Taxonomy,Organization,Address,City,
    State,Zip,Contact,Email,Office_phone,Mobile_phone,Fax
FROM physician
where Phy_id = '".$Phy_id."' "; 
...
$sql_notes_general="SELECT NG.id_notes_general,NG.notes_general,NG.date_notes,NG.id_person,DTP.type_persons,US.Last_name,US.First_name,P.Name FROM tbl_notes_general NG LEFT JOIN tbl_doc_type_persons DTP ON (DTP.id_type_persons = NG.id_type_person)
                    LEFT JOIN physician P ON (P.phy_id=NG.id_person)
                    LEFT JOIN user_system US ON (US.user_id = NG.id_user) WHERE NG.table_reference='physician' AND NG.id_person=".$Phy_id." ORDER BY NG.id_notes_general ASC";
...
left join tbl_contactos using(id_contactos)
            WHERE
        tabla_ref = 'physician' and id_tabla_ref = '".$Phy_id."' GROUP BY persona_contacto;"; 
...
$sqlDocuments = "
  ...
  LEFT JOIN physician p on p.Phy_id = ct.id_person
  WHERE true AND ct.id_type_person = 4 AND ct.id_person = '".$Phy_id."';
...
<body> 
  ...
  if(!isset($_GET['name'])){?>
            <div class="col-md-12"> 
             <form class="form-horizontal" id="myForm" action="search_physician.php" method="post"> 
            
            <div class="col-lg-12">
                <div class="col-xs-1"></div> <h3 class="page-header">Choose Physician from List</h3>
            </div> 
            <div class="row">
            ...
            <div class="col-xs-3">
                ...
                <select style="width:200px;" name='name' id='name'  onchange="blockCheckBox();">
                    <option value=''>--- SELECT ---</option>                 
                    <?php 
                  $sql  = "Select NPI, Phy_id, Name as Physician_name from physician order by Name "; 
                    $conexion = conectar(); 
      ...
      if((trim($row["Phy_id"])) == (trim($patient_id)."-".$company)) 
                            print("<option value='".$row["Phy_id"]."' selected>".$row["Physician_name"]." </option>"); 
                        else 
                            print("<option value='".$row["Phy_id"]."'>".$row["Physician_name"]." </option>"); 
                ...
                <div class="col-xs-3">
                <input id='find' style="height: 33px" name='find' type='submit' value=" Find " class="btn btn-success " onclick="findData();">
                &nbsp&nbsp           <input style="height: 33px" name='reset' type='button' value=" Reset " onclick= "window.location.href = 'search_physician.php';" class="btn btn-danger ">
                </div>
                                        ...
                                        if($_POST['name'] != ''){  
                                            echo '
                                                    <tr>
                                                       ...
                                                       <td><a onclick="showPhysician(\''.$row['id_person'].'\')">'.strtoupper($row['Name']).'</a></td>
      ...
      <div class="panel-body"> 
        ...
        $conexion = conectar();
            ...
            while (isset($reporteDocuments[$t])){ 
                    ...
                    $datos_result .= '<td>'.$reporteDocuments[$t]['Name'].'</td>';                    
...
<!-- ########################################################################################################## --> 
 ...
 <div class="row">
            <div class="col-lg-12 text-center"><b><h4>PHYSICIANS LIST</h4></b></div>    
        </div>
...
<tbody>
      ...
      $sqlpatientslist = " SELECT Phy_id ,Name,NPI,address,Office_phone,Mobile_phone from physician ";
...
echo '
        <tr>
                      ...
                      <td><a onclick="showPhysician(\''.$row['Phy_id'].'\')">'.$row['Phy_id'].'</td> 
                      <td><a onclick="showPhysician(\''.$row['Phy_id'].'\')">'.$row['Name'].'</td> 
                      <td>'.$row['NPI'].'</td>
                      <td>'.$row['address'].'</td> 
                      <td>'.$row['Office_phone'].'</td> 
                      <td>'.$row['Mobile_phone'].'</td> 
