...
$sql_case_tree_count="Select SN.id_note,COUNT(SN.id_note) as contador_test From calendar_appointments C  LEFT JOIN tbl_visits V ON(V.app_id=C.id) and V.deleted='0' and V.id_visit_type='2' LEFT JOIN tbl_notes_documentation ND ON(ND.visit_id=V.id) JOIN tbl_signature_note SN ON(SN.id_note=ND.id_notes)  LEFT JOIN calendar_appoitment_date CD ON (C.id=CD.id)  WHERE date(CD.start)< now()  GROUP BY SN.id_note ";
...
LEFT JOIN calendar_appoitment_date CD ON (C.id=CD.id)
WHERE EM.assistant='1' and date(CD.start)< now()  GROUP BY SN.id_note ";
