...
$sql_case_tree="Select C.id,EM.*,concat(EM.first_name,' ',EM.last_name) as user1,EM.assistant,CD.start as date_appoitment,P.*,concat(P.First_name,' ',P.Last_name) as patient,D.Name as discipline From calendar_appointments C  LEFT JOIN tbl_visits V ON(V.app_id=C.id) and V.deleted='0' and V.id_visit_type='2' LEFT JOIN tbl_notes_documentation ND ON(ND.visit_id=V.id) JOIN tbl_signature_note SN ON(SN.id_note=ND.id_notes) LEFT JOIN employee EM ON(EM.id=C.therapist_id) LEFT JOIN calendar_appoitment_date CD ON (C.id=CD.id) LEFT JOIN patients P ON (P.id = C.Pat_id) LEFT JOIN discipline D ON (D.DisciplineId=EM.discipline_id) WHERE     date(CD.start)< now() ";
#AND COUNT='0'
...
$sql_case_four ="SELECT C.id,EM.*,concat(EM.first_name,' ',EM.last_name) as user1,EM.assistant,CD.start as date_appoitment,P.*,concat(P.First_name,' ',P.Last_name) as patient,D.Name as discipline From calendar_appointments C  LEFT JOIN employee E ON(C.therapist_id=E.id) LEFT JOIN tbl_visits V ON(V.app_id=C.id) LEFT JOIN tbl_notes_documentation ND ON(ND.visit_id=V.id) JOIN tbl_signature_note SN ON(SN.id_note=ND.id_notes) LEFT JOIN employee EM ON(EM.id=C.therapist_id) LEFT JOIN calendar_appoitment_date CD ON (C.id=CD.id) LEFT JOIN patients P ON (P.id = C.Pat_id) LEFT JOIN discipline D ON (D.DisciplineId=EM.discipline_id) WHERE E.assistant='1' AND V.deleted='0' AND V.id_visit_type='2' AND date(CD.start)<now()";
#count<2
...
$sql_case_one_rdos="Select C.id,COUNT(C.id) as coun  from calendar_appointments C   JOIN calendar_appoitment_date CAD ON(CAD.calendar_appoitment_id =C.id) WHERE C.attendance != '4' AND C.Pat_id != 'NULL'  AND CAD.start< now() GROUP BY C.id ORDER BY coun desc ";
#AND V.id IS NULL
...
<head>
    ...
    <div  align="center">
        <button id="boton_primer_reporte" type="button" class="btn btn-primary" data-toggle="button" aria-pressed="false" autocomplete="off" >Primer repoter <span class="badge" style="color: #ff0000"><?php echo $total?></span></button>
