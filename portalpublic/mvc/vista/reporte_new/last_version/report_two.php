...
for($j=0;$j<count($case_dos);$j++) {
    $sql_case_one = "Select C.id as c_id,P.DOB,P.*,concat(P.Last_name,'.',P.First_name) as pat_name,P.phone  from calendar_appointments C   JOIN calendar_appoitment_date CAD ON(CAD.calendar_appoitment_id =C.id) LEFT JOIN patients P ON (P.id=C.Pat_id) WHERE C.id='" . $case_dos[$j]['id'] . "' ";
#AND V.id IS NULL
    $resultado = ejecutar($sql_case_one, $conexion);
