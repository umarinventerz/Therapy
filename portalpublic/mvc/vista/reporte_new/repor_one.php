...
{
    $sql_reptree_case_one="Select C.id,P.*,concat(P.Last_name,'  ',P.First_name) as pat_name3,CD.start as date_appoitment From calendar_appointments C LEFT JOIN employee EM ON (C.therapist_id=EM.id)  LEFT JOIN tbl_visits TV ON (TV.app_id=C.id) LEFT JOIN patients P ON (P.id=C.Pat_id) LEFT JOIN calendar_appoitment_date CD ON (C.id=CD.calendar_appoitment_id) WHERE EM.id='".$id3."' AND TV.deleted='0' AND C.attendance='4' AND C.Pat_id != 'NULL' AND date(CD.start)<now()";
