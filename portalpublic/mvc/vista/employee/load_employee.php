...
if(isset($_POST['submit']) == 'submit' && ($_POST['action'] != 'edit_submit' && $_POST['action'] != 'edit')||
isset($_POST['submit1']) == 'submit1' && ($_POST['action'] != 'edit_submit' && $_POST['action'] != 'edit')){
	//quite el codigo de subir lor archivos 
...
$input_date_licence =date("Y-m-d",strtotime($input_date_licence));
$fingerPrint=$_POST['fingerPrint']; 
list($mm,$dd,$aa) = explode('/',$_POST['input_date_finger']);
$input_date_finger = $aa.'-'.$mm.'-'.$dd; 
$input_date_finger =date("Y-m-d",strtotime($input_date_finger));
...
if ($_POST['input_date_licence']=='') {
    $Intake_Agmts = date("Y-m-d",strtotime($_POST["input_date_licence"]));
}
...
values ('".$lastName."','".$firstName."','".$userName."','".$password."','1','".$user_type."');";
...
$i = 2;
//quite la insercion de archivos en la db que igual no pinchaba
...
$insert_barcodes="INSERT into tbl_barcodes (`id_relation`,`barcode`,`id_type_person`) 
...
echo '<script>window.location="../documents/documents.php";</script>';
 }
echo "<script type=\"text/javascript\">alert(\"Employee Save\");</script>";
echo '<script>window.location="load_employee.php";</script>';
	...
	if($_POST['submit'] == 'submit' && $_POST['action'] == 'edit'|| isset($_POST['submit1']) == 'submit1' && $_POST['action'] == 'edit'){
        // esto es lo mismo de subir archivos que no pinchaba
     /*   $carpeta = 'employee/'.$_POST['firstName'].'_'.$_POST['lastName'];
        if (!file_exists("../../../".$carpeta)) {
            mkdir("../../../".$carpeta, 0777, true);
        }
    ...
    //subimos el fichero a la carpeta si no existe la creamos
        if (!file_exists("../../../images/avatar")) {
            mkdir("../../../images/avatar", 0777, true);
        }
    $nombre="avatar/".uniqid('cupon-') . '-foto.jpg';
...
//insertar el avatar en la base de datos
       ...
       $buscar="SELECT * FROM avatar WHERE identificador = ".$_POST['id_employee']."";
       $busqueda=ejecutar($buscar,$conexion);
       $row_cnt = mysqli_num_rows($busqueda);
       if ($row_cnt>0 &&  $source!='') {
           $update_avatar=" UPDATE avatar SET `tipe_user` = '".$kindEmployee."',`picture` = '".$nombre."',`updated_at` = '".$fecha."'"
                . "WHERE identificador = ".$_POST['id_employee'].";";
       }if( $source!=''){
        $update_avatar="INSERT into avatar (`tipe_user`,`picture`,`identificador`,`created_at`,`updated_at`) 
values ('".$kindEmployee."','".$nombre."','".$_POST['id_employee']."','".$fecha."','".$fecha."');";}
        else{
...
ejecutar($update_avatar,$conexion);
    ...
    $user_type=3;
        ...
        $update_user_system=" UPDATE user_system SET `user_name` = '".$userName."',`password` = '".$password."' ,`User_type` = '".$user_type."'"
	...
	if(isset($treatment[0][0])){
        ...
        $i = 2;	
        // essto es lo mismo de insertar la ruta en la db
      /*  while(isset($route_file[$i])){
                $insertDocuments = "INSERT into tbl_documents (id_type_document, id_type_person, id_table_relation, id_person , route_document) 
...
if($_POST['action'] == 'edit_submit'){
	...
	$result01 = mysqli_query($conexion, $max_id);		
	//die();
        $avatar="select picture from avatar WHERE identificador=".$idEmployee;
...
<script src="../../../js/main.js"></script>
    ...
    function unlockNextField(nextfield,value,field){
        ...
        if(field != ''){
            ...
            if(nextfield == 'credentials' && $("#kindEmployee").val() == 'Administrative')
                nextfield = 'email1';
...
<div class="row">
		...
		<div class="box">			
			<div class="box-content">
				...
				if($_POST['action'] == 'edit_submit'){
					...
					$name_form="Registration form";
				}
				?>
				<h4 class="page-header"><?php echo $name_form; ?></h4>
				<form  role="form" enctype="multipart/form-data" class="form-horizontal" action="load_employee.php" method="post">
					<?php if($_POST['action'] != ''){
						$disabled = '';
					?>					
					<input type="hidden" id="action" name="action" value="<?php echo ($_POST['action'] == 'edit_submit')?'edit':'insert';?>">
  ...
  <div class="col-sm-2 text-right">
              <img class="img-responsive" id="image" src="<?php echo $nombreavatar;?>" width="150" height="150" />
            </div>             </div>
								...
								<?php
								 $sql  = "Select * from employee_role order by id asc"; 
                                                              ...
                                                              {   
                                                                        ...
                                                                        if($row["role"]==$kind_Employee){
                                                           print("<option value='".$row["role"]."' selected>".$row["role"]."</option>"); 
                                                                        }else{
...
<?php
                ...
                ?>
                            <input type="file"  id="Photo" src="" onchange="filePreview(this)" name="Photo"  placeholder="Photo"></div>     
                                                                                             
					</div>                                        
					<div class="form-group">
						<label class="col-sm-2 control-label">First name</label>
						<div class="col-sm-4">
							<input type="text" required="required" onkeyup="unlockNextField('lastName',this.value,'');" value="<?php echo $first_name;?>" class="form-control" placeholder="First name" data-toggle="tooltip" data-placement="bottom" title="Tooltip for name" name="firstName" id="firstName">
						</div>
						<label class="col-sm-2 control-label">Last name</label>
						<div class="col-sm-4">
							<input type="text" required="required" onkeyup="unlockNextField('userName',this.value,'');" value="<?php echo $last_name;?>" class="form-control" placeholder="Last name" data-toggle="tooltip" data-placement="bottom" title="Tooltip for last name" name="lastName" id="lastName" <?php echo $disabled;?>>
						</div>
					</div>
                                        <div class="form-group">
						<label class="col-sm-2 control-label">User name</label>
						<div class="col-sm-4">
							<input type="text" required="required" onkeyup="unlockNextField('password',this.value,'');" value="<?php echo $user_name_text;?>" class="form-control" placeholder="User name" data-toggle="tooltip" data-placement="bottom" title="Tooltip for name" name="userName" id="userName" <?php echo $disabled;?>>
						</div>
						<label class="col-sm-2 control-label">Password</label>
						<div class="col-sm-4">
							<input type="password" required="required" onkeyup="unlockNextField('payTo',this.value,'');" value="<?php echo $password_text;?>" class="form-control" placeholder="password" data-toggle="tooltip" data-placement="bottom" title="Tooltip for last name" name="password" id="password" <?php echo $disabled;?>>
						</div>
					</div>
					...
					<div class="form-group" id="divCredentialsEmail">						
						...
						<div class="col-sm-4">
							<input type="email" onkeyup="unlockNextField('licenceNumber',this.value,'');unlockNextField('input_date_licence',this.value,'');" value="<?php echo $email;?>" class="form-control" name="email" id="email" <?php echo $disabled;?>/>
						</div>						
					</div>
					<div class="form-group" id="divEmailAdministrativo" style="display:none;">
						<label class="col-sm-2 control-label">Email address</label>
						<div class="col-sm-4">
							<input type="email" onkeyup="unlockNextField('licenceNumber',this.value,'');unlockNextField('input_date_licence',this.value,'');" value="<?php echo $email;?>" class="form-control" name="email" id="email1" <?php echo $disabled;?>/>
						</div>						
					</div>
					...
					<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-2">
								...
								<div class="radio">
									<label>
										<input type="radio"  onclick="showTimePay();" value="Salary" name="typeSalary" id="typeSalary1"
                                         <?php if($type_salary == 'Salary') echo 'checked="true"';?>/> Salary 
										<i class="fa fa-square-o small"></i>
									</label>
								</div>
							</div>
					...
					<div class="form-group" id="divAmountTreatment" style="display:none;">
						...
						<div class="col-sm-2">
							<div >
								<label>
									<input onkeyup="unlockNextField('civilStatus',this.value,'');" type="number" value="<?php echo $tx_in;?>" name="amount_tx_in" id="amount_tx_in" <?php echo $disabled;?>/>
								</label>
							</div>
						</div>
						<div class="col-sm-2">
							<div >
								<label>
									<input onkeyup="unlockNextField('civilStatus',this.value,'');" type="number" value="<?php echo $tx_out;?>" name="amount_tx_out" id="amount_tx_out" <?php echo $disabled;?>/>
								</label>
							</div>
						</div>
						<div class="col-sm-2">
							<div>
								<label>
									<input onkeyup="unlockNextField('civilStatus',this.value,'');" type="number"  value="<?php echo $eval_in;?>" name="amount_evals_in" id="amount_evals_in" <?php echo $disabled;?>/>
								</label>
							</div>
						</div>						
						<div class="col-sm-2">
							<div>
								<label>
									<input onkeyup="unlockNextField('civilStatus',this.value,'');" type="number"  value="<?php echo $eval_out;?>" name="amount_evals_out" id="amount_evals_out" <?php echo $disabled;?>/>
								</label>
							</div>
						</div>
						<div class="col-sm-2">
							<div>
								<label>
									<input onkeyup="unlockNextField('civilStatus',this.value,'');" type="number"  value="<?php echo $transportation;?>" name="amount_transportation" id="amount_transportation" <?php echo $disabled;?>/>
								</label>
							</div>
						</div>
					</div>
					...
					<div class="form-group" id="divAmountSalary" style="display:none;">
						...
						<div class="col-sm-2">
							<div >
								<label>
									<input onkeyup="unlockNextField('civilStatus',this.value,'');" type="number" value="<?php echo $amount_salary;?>" name="amount_salary" id="amount_salary" <?php echo $disabled;?>/>
								</label>
							</div>
						</div>											
					</div>
					...
					<div class="form-group">
						...
						<div class="col-sm-4">
							<select onchange="unlockNextField('submit',this.value,'');unlockNextField('submit1',this.value,'');" class="populate placeholder" name="status" id="status" <?php echo $disabled;?>>
				...
				<!--	<div class="row fileupload-buttonbar">
					...
					<div class="form-group">
						...
						<div class="col-sm-2">
							<button type="submit" onclick="" class="btn btn-primary btn-label-left" value="submit" name="submit" id="submit" <?php echo $disabled;?>>
							<span><i class="fa fa-clock-o"></i></span>
								Submit
							</button>
						</div>
                        <div class="col-sm-2">
                            <button type="submit" class="btn btn-success btn-label-left" value="submit1" name="submit1" id="submit1" <?php echo $disabled;?>>
                            <span><i class="fa fa-clock-o"></i></span>
                                Upload Documents
                            </button>
