...
@Component({
    ...
    directives: [jqxPopoverComponent, jqxButtonComponent]
})

export class AppComponent implements AfterViewChecked
{
    @ViewChild(jqxPopoverComponent) myPopover: jqxPopoverComponent;
    @ViewChild(jqxButtonComponent) button: jqxButtonComponent;

    flag: Boolean = false;
    settings: jqwidgets.PopoverOptions;
    ...
    generateItems = function (domID)
    {
        ...
        var itemsContainer = <HTMLElement>document.getElementById(domID);
    ...
    public ngAfterViewChecked(): void
    {
        if (this.flag === false)
        {
            var self = this;
            self.Initialize();
            (<HTMLElement>document.getElementsByTagName('angularButton')[0].firstElementChild).style.padding = '8px 0px 0px 0px';
    ...
    {
        // prepare the data
        var selectorName: string = 'movies';
